      ...
      checkLoginStatus({token})
        .then(res => {
          if (res.isLoggedIn) {
            ...
            const prefectureArr = Object.keys(prefecture).map(
              key => prefecture[key]
            );
            Database.AddPrefecture(prefectureArr)
              .then(res => {
                if (res) {
                  response.send(200, {
                    success: true,
        ...
        .then(res => {
          if (res.isLoggedIn) {
            ...
            Database.GetPrefecture(id)
              ...
              .then(dateArr => {
                Database.UpdatePrefecture(id, dateArr)
                  .then(res => {
                    if (res) {
                      response.send(200, {
                        success: true,

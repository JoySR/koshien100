    ...
    if (request.body) {
      ...
      checkLoginStatus({token}).then(res => {
        if (res.isLoggedIn) {
          ...
          Database.AddArea(areaArr).then(res => {
            if (res) {
              response.send(200, {
                success: true,
      ...
      checkLoginStatus({token}).then(res => {
        if (res.isLoggedIn) {
          ...
          Database.GetArea(id).then(res => {
            ...
            Database.UpdateArea(id, dateArr).then(res => {
              if (res) {
                response.send(200, {
                  success: true,
    ...
    if (request.body) {
      ...
      checkLoginStatus({token}).then(res => {
        if (res.isLoggedIn) {
          ...
          Database.RemoveArea(id).then(res => {
            if (res) {
              response.send(200, {
                success: true,

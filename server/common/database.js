...
const Database = {
  ...
  CheckSession: token => {
    const sql =
      'SELECT * FROM `sessions` WHERE `token` = ' + pool.escape(token);
    return new Promise((resolve, reject) => {
      pool.query(sql, (err, rows, fields) => {
        if (err) reject(err);
        else resolve(rows);
  ...
  AddDate: date => {
    ...
    return new Promise((resolve, reject) => {
      pool.query(sql, [date], (err, rows, fields) => {
        if (err) reject(err);
        else resolve(rows);
      });
    });
  ...
  UpdateDate: (id, date) => {
    const sql =
      'UPDATE dates set date_id = ? , game_date = ? , status = ? WHERE id = ' +
      pool.escape(id);
    return new Promise((resolve, reject) => {
      pool.query(sql, date, (err, rows, fields) => {
        if (err) reject(err);
  ...
  AddArea: area => {
    ...
    return new Promise((resolve, reject) => {
      pool.query(sql, [area], (err, rows, fields) => {
        if (err) reject(err);
        else resolve(rows);
      });
    });
  ...
  UpdateArea: (id, area) => {
    const sql =
      'UPDATE areas set area_id = ? , name = ? , code = ?  WHERE id = ' +
      pool.escape(id);
    return new Promise((resolve, reject) => {
      pool.query(sql, area, (err, rows, fields) => {
        if (err) reject(err);
  ...
  AddGame: game => {
    const sql =
      'INSERT INTO games (game_id, date_id, round, time, is_first_home, first_id, third_id, first_scores, third_scores, video_id) VALUES (?)';
    return new Promise((resolve, reject) => {
      pool.query(sql, [game], (err, rows, fields) => {
        if (err) reject(err);
        else resolve(rows);
      });
    });
  ...
  UpdateGame: (id, game) => {
    const sql =
      'UPDATE games set game_id = ? , date_id = ? , round = ? , time = ? , is_first_home = ? , first_id = ? , third_id = ? , first_scores = ? , third_scores = ?, video_id = ? WHERE id = ' +
      pool.escape(id);
    return new Promise((resolve, reject) => {
      pool.query(sql, game, (err, rows, fields) => {
        if (err) reject(err);
  ...
  GetPrefecture: id => {
    const sql =
      'SELECT * FROM `prefectures` WHERE `prefecture_id` = ' + pool.escape(id);
    return new Promise((resolve, reject) => {
      pool.query(sql, (err, rows, fields) => {
        if (err) reject(err);
        else resolve(rows);
  ...
  AddPrefecture: prefecture => {
    const sql =
      'INSERT INTO prefectures (prefecture_id, name, area_id, code) VALUES (?)';
    return new Promise((resolve, reject) => {
      pool.query(sql, [prefecture], (err, rows, fields) => {
        if (err) reject(err);
        else resolve(rows);
      });
    });
  ...
  UpdatePrefecture: (id, prefecture) => {
    const sql =
      'UPDATE prefectures set prefecture_id = ? , name = ? , code = ? , area_id = ?  WHERE id = ' +
      pool.escape(id);
    return new Promise((resolve, reject) => {
      pool.query(sql, prefecture, (err, rows, fields) => {
        if (err) reject(err);
  ...
  RemovePrefecture: id => {
    const sql =
      'DELETE FROM `prefectures` WHERE `prefecture_id` = ' + pool.escape(id);
    return new Promise((resolve, reject) => {
      pool.query(sql, (err, rows, fields) => {
        if (err) reject(err);
        else resolve(rows);
